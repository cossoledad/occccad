...
4. 基准几何是屏幕空间辅助对象。三条正半轴和位于正象限的三个小基准面保持固定屏幕尺寸、关闭深度测试并始终可选，不随模型包围盒放大；面片与轴线保留屏幕空间间隙，轴线采用专用虚线及较小命中容差。一旦通过精确容差命中，基准几何所属 interaction layer 高于实体拓扑，避免前景实体抢走 hover/selection。
...
- Instance 显示 `ReferenceName(InstanceName)`；分支节点收起时显示单个圆环，展开时以独立 SVG 路径显示向四角分离 1.75 px 的四个四分之一圆弧，不使用平台折叠箭头或中心十字。
...
- 结构树右上角提供可键盘操作的 24 px `┓` 形 resize grip，宽度限于 220–640 px 并跨会话记忆；透明树容器不截获空白区域指针，只有节点行和调宽 grip 接管输入，其他位置继续交给三维视口。
- 文档新建/切换/关闭入口从视口左下角迁入全局标题栏，并保持单图标按钮，避免遮挡模型、挤占标题栏或受 Header 行高影响错位。
