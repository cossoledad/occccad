...
| LARGE-COMPUTE（剩余） | 已交付 parse-once、Solid 拆分与有界并发；剩余组件 checkpoint、小摘要响应、精确/显示分阶段、预算调度与进程隔离；原生长求值 Jobs | 不再每 root 重读源；无全量 mesh RPC/结果数组积压；OOM/取消/重领/CAS/隐藏候选发布；不超资源预算 |
