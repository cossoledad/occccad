...
M1.7 的版本化 `AssemblySolverProfile` 已由 Go Client 经 `SolveAssemblyRequest` 传到 Worker，除原有 convergence/classification、step、degeneracy 和 damping 外，还包含独立 translation/rotation central-difference step、SVD absolute/relative rank threshold、gradient stationary threshold、cluster motion-preference weight 与 bounded conflict-probe budget。旧 `finite_difference_step`/`rank_tolerance` 字段保留为兼容 override；`schema_version` 当前必须为 1，零值沿用 kernel 默认值。响应会返回 constraint effective/incremental rank、directed-angle wrapped/unwrapped/winding 以及 suspected conflict IDs。
