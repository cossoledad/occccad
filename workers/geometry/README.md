...
Persistent topology naming 已完成 P0–P2。公共值类型与 Proto 包含 `PersistentSelection`、`TopologyHistory`、resolution、tombstone/ambiguity，以及 Supporting Element 和 Assembly Constraint 两个独立状态域。选择证据中的 measure 使用可选 SI 值并显式携带量纲；policy 的线性与角度容差分别使用米和弧度。Linear Extrude 使用 region/entity ID 生成 `START_CAP`、`END_CAP` 和 `SIDE_FROM_PROFILE_EDGE`；Boolean 与 same-domain unify 的 Generated/Modified/Unchanged/Split/Merged/Deleted 证据被折叠到逐 FeatureResult，并以最终 Shape local ID、几何 signature、adjacency 和 shape gate 校验。`PartEvaluationManifest` 同时携带摘要与不可变 protobuf topology manifest 的 digest/ArtifactReference；Product resolver 仍属于 P3，当前结果不能单独宣称跨 Revision 选择已经可用。
