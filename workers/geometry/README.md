...
P7 的 evaluator `occccad.topology.contract.v2` 进一步为 Linear Extrude 输出 `START/END_BOUNDARY_FROM_PROFILE_EDGE`、`VERTICAL_FROM_PROFILE_ENDPOINTS` 和 `START/END_VERTEX_FROM_PROFILE_ENDPOINTS`。共享角点的身份来自排序后的 Sketch endpoint ID 集，不来自坐标或 OCCT 遍历号；数值曲线和点特征只作为 evidence。Edge evidence 明确携带参数区间与 cap/vertical/derived 角色，Vertex evidence 携带 cap/derived 角色。Boolean 和 `ShapeUpgrade_UnifySameDomain` 映射保留 source topology type，Shape gate 同时覆盖最终 Face、Edge、Vertex。Worker 序列化每个 output 的实际枚举类型；矩形拉伸契约固定为 6 Face、12 Edge、8 Vertex。policy digest 已随 evaluator 变化，使 v1 制品和缓存不会被 v2 请求采用。C++ corpus 覆盖长度编辑、Boolean merge/split/delete、贯穿孔、圆环 seam 和容差以下短边；控制面再从 artifact 精确读取 Vertex Point 与 linear Edge Axis，不由 Worker 承担 Product 身份或约束状态。
