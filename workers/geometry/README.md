...
M1.6 的版本化 `AssemblySolverProfile` 已由 Go Client 经 `SolveAssemblyRequest` 传到 Worker，包含 max iterations、length/angle convergence tolerance、独立 classification tolerance、translation/rotation step tolerance、degeneracy threshold、finite-difference step、initial damping 和 rank tolerance。`schema_version` 当前必须为 1；字段为零时沿用 Worker/kernel 默认值，未知版本返回 `INVALID_ARGUMENT`。
