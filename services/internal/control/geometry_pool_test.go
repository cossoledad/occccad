...
func TestGeometryPoolRoutesPartNamingContractWithoutDroppingIdentity(t *testing.T) {
	...
	response, err := workerv1.NewGeometryWorkerClient(connection).EvaluatePart(t.Context(), &workerv1.EvaluatePartRequest{
		...
		TopologyPolicy: &workerv1.TopologyNamingPolicy{SchemaVersion: 1,
			PolicyId: "occccad.topology.naming.v1", EvaluatorVersion: "occccad.topology.contract.v3",
...
func TestGeometryPoolRoutesSolveAssembly(t *testing.T) {
	...
	response, err := workerv1.NewGeometryWorkerClient(connection).SolveAssembly(t.Context(), &workerv1.SolveAssemblyRequest{
		...
		SolverProfile: &workerv1.AssemblySolverProfile{SchemaVersion: 2,
			LengthTolerance: 1e-7, ClassificationLengthTolerance: 1e-6},
	})
	if err != nil {
		t.Fatalf("SolveAssembly was not routed: %v", err)
	}
	if response.GetStatus() != "CONVERGED" || response.GetBodies()[0].GetId() != "part-1" || response.GetClassification() != "SOLVED_UNDER_CONSTRAINED" || response.GetComponents()[0].GetGaugeDof() != 6 || response.GetDiagnostic() != "MOVE_FIRST_MINIMIZE_REFERENCE/profile-v2" {
		t.Fatalf("unexpected routed response: %#v", response)
	}
}
