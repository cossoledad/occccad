...
func Migrate(ctx context.Context, pool *pgxpool.Pool) error {
	if _, err := pool.Exec(ctx, `CREATE SCHEMA IF NOT EXISTS occccad`); err != nil {
	...
	for _, entry := range entries {
		...
		if err := pool.QueryRow(ctx,
			`SELECT EXISTS(SELECT 1 FROM occccad.schema_migrations WHERE version=$1)`,
		...
		if _, err = tx.Exec(ctx, string(sql)); err == nil {
			_, err = tx.Exec(ctx,
				`INSERT INTO occccad.schema_migrations(version) VALUES($1)`, entry.Name())
		}
		if err != nil {
			_ = tx.Rollback(ctx)
