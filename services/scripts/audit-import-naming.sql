...
WITH imported AS (
  SELECT d.id, v.id AS revision_id, d.head_version_id = v.id AS current_head,
         v.geometry_key, a.geometry_key AS artifact_key, a.topology_manifest_digest AS digest,
         a.topology_manifest_object_id AS object_id,
         octet_length(a.topology_manifest_data) AS inline_bytes
...
SELECT CASE WHEN current_head THEN 'HEAD' ELSE 'HISTORY' END AS snapshot,
       CASE WHEN geometry_key IS NULL THEN 'NO_GEOMETRY'
            ...
            WHEN NULLIF(trim(digest),'') IS NULL AND object_id IS NULL AND COALESCE(inline_bytes,0)=0 THEN 'NAMING_ABSENT'
            WHEN NULLIF(trim(digest),'') IS NULL THEN 'MISSING_DIGEST'
            WHEN object_id IS NULL AND COALESCE(inline_bytes,0)=0 THEN 'MISSING_CONTENT'
