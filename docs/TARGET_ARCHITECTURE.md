...
当前已落地的 M1/M1.5/M1.6 算法基线位于 `kernel/assembly`：它以纯值类型表达刚体和 Point/Axis/Plane/Cylinder descriptor，先合并 Rigid cluster、消元 Fix/Ground，再按 constraint graph connected component 在 `SE(3)` 局部增量上执行 dense 有限差分阻尼最小二乘；结果包含 equation provenance、Jacobian rank、relative/gauge DOF、整块冗余和最终未满足诊断。request-scoped SolveIntent 可以在无物理 ground 时以第二选择的 reference cluster 消除全局 gauge，但还不是已接地系统中的严格层级最小位移。该库仍刻意不依赖 OCCT、Product 和持久拓扑标识；首个 Product adapter 已能把直接 Part occurrence 的 Datum/AxisSystem 引用以及当前 Revision 的平面/圆柱面选择解析为局部描述符，并经正式 Router 的 `SolveAssembly` RPC 提交权威 Pose。面选择目前以 geometryKey 和 topology local ID 绑定不可变制品并由服务端重新查询 OCCT 属性；它不是跨特征重算的稳定命名，仍须演进为 PersistentSelection/Publication adapter，且不能由前端坐标代替。下一门槛是 M2 的解析 Jacobian、SVD/QR rank、null-space basis 和可解释 DOF；层级 minimum-motion 与持久 branch intent 属于 M3。近期不把 sparse backend 或 MUS 搜索置于上述正确性工作之前。

M1.6 鲁棒性门现已落地：使用 `atan2` 稳定零度与 π 附近的无向角；在一次 solve 内冻结方向和距离侧分支；分离长度、角度、平移/旋转步长、退化与 rank 容差；近平行直线距离使用连续退化极限；并区分 `Unsatisfied/Inconsistent` 与 `MaxIterations/NumericalFailure/NonConvergent`。下一门槛为 M2 的解析 Jacobian、SVD/QR rank、null-space basis 和可解释 DOF；持久 branch intent 与层级 minimum-motion 属于 M3。
