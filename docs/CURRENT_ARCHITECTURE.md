...
Skeleton 第一版仍是普通 Part。`ContextReference` 保存 owning Workspace、root Product Revision、source/owning InstancePath、Context Variant、Publication/PersistentSelection、resolved descriptor 与到 owning Part frame 的 Transform。PLANE/AXIS 物化为本地 Datum，PARAMETER 进入外部参数快照，CURVE 进入权威 Sketch ExternalGeometry 投影；Isolate 分别固化为 literal、Datum 或普通 construction sketch entity。跨文档依赖保持单向 DAG，共享 Part 的 occurrence-specific context 必须显式给出 path/variant。当前 Product Publication 转发只接受一个相对 occurrence segment；嵌套 rigid Product 与完整 path rewrite 属于 P10。
...
| Publication、跨文档更新与 Skeleton | 已实现 P9A–P9G 自动化基线 | Part/Product Publication、冻结 resolution snapshot、显式 Update References/CAS/Undo、装配 Publication endpoint、兼容 Replace、ContextReference Datum/Parameter/Curve、DAG/variant gate、Isolate、结构树/Inspector/管理面板；P10 再扩展嵌套 typed InstancePath 与 SolveManifest |
...
上述装配路径现已接入 P9 Publication endpoint：约束保存 PublicationRef、合同与 PersistentSelection deep link，Product 可转发单层 occurrence Publication；P10 继续负责嵌套 typed InstancePath 与可重放 SolveManifest。
