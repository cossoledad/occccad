...
视口默认使用正交投影与等轴测方向（世界方向 1/-1/1），显示比例由正交 zoom 管理。等轴测命令（快捷键 1）先设置标准 ISO 朝向，再按可见内容包围盒适合窗口；其他标准方向保留当前比例。独立适合窗口命令保留当前朝向。平移、点线拾取和装配操纵器共用当前投影的 CSS 像素/世界单位换算；指针锚定缩放同时维护相机与 pivot，避免高倍缩放后围绕旧中心旋转。Default/CATIA 在局部放大时使用当前可见几何作为局部旋转参考，完整模型可见时仍保持整体导航。进入基准面/实体平面草图时保留比例并对齐支撑平面，退出恢复进入前的相机姿态、中心与 zoom；同一草图的重复刷新不重置视图。视图快照只属于本地交互，不写入 Revision。新文档初次打开按可见内容适合窗口，普通模型刷新不改变视图。正交 near=0，far 按几何视深与视图尺寸自适应；缩小后再放大会沿视线前移相机并收回多余深度范围，避免操作历史损害深度精度；必要时沿视线后移相机以容纳眼后几何，不改变屏幕投影。底部 XY 网格为独立背景通道中的无限投影网格，摆脱固定 1200 尺寸和模型裁剪面，随缩放调整密度，不参与 Fit/拾取。

草图会话提供 `sketch.normal`（正对草图平面）视图命令，保留当前 zoom/关注区域，不重置进入前快照。退出草图不清空视口中的权威 UI 选择投影；空白取消始终通知工作台，即使内部已为空。场景重建保留同文档选择；已被实体特征使用的草图被选中时临时显示其几何高亮，取消后恢复原显隐规则，不修改用户显隐偏好。草图无限网格按支撑平面 origin/u/v/normal 投影，世界网格持续保留；两者按 1/2/5 十进制间距维持可读密度。背景渐变采用世界 Z 向上环境的 Zenith/Horizon/Nadir 插值，视图俯仰和滚转改变背景方向，平移与缩放不改变背景环境方向。此为参照 3DEXPERIENCE ambience 文档的应用实现，不声明专有 shader 数值完全一致。

工作台数值输入首次获得焦点时全选，后续点击可定位光标。长度纯数字按当前有效文档显示单位解释，显式单位仍覆盖默认值，表达式仍交由权威求值器处理；提交前转换为原有 API 的毫米值，不改变持久单位合同。结构树名称按面板剩余宽度截断，不设固定 250px 上限。顶部文档页签支持水平拖动、位移动画、Alt+左右键排序与浏览器会话顺序恢复，拖动不切换激活文档。3DE 平移、旋转和组合键缩放增益在原实现上统一提高 15%，仍沿用缩放相关的屏幕精度换算。

实体特征预览显示后端求值的完整结果，按新建/融合/切除/交集区分提示色；暂时隐藏被替代的原实体，以淡色表面和虚线轮廓提供原始参考，使用正常深度遮挡避免透视线条淹没结果。取消/替换预览恢复原实体可见性；装配中仅处理当前编辑 occurrence。此显示不推断新增/删除面的稳定拓扑分类。草图无限网格采用青蓝点阵细分与连续主线，世界底面保持灰色连续网格。

鼠标导航支持 Default、3DEXPERIENCE CATIA 原生应用 和 SOLIDWORKS Part/Assembly 三种 Profile。CATIA 顺序组合键、Ctrl+中键、两键替代操作由独立状态机处理，当前采用组合键缩放且禁用三维滚轮；依原生 Navigation 帮助默认隐藏旋转球，可通过 UI preferences 开启并持久化。可选虚线球面导引与轨迹球共享屏幕半径；V5 两键替代作为兼容扩展，Robot/Shift 视点操纵尚未实现。SOLIDWORKS 独立处理中键旋转、Ctrl 平移、Shift 缩放、Alt 滚转、中键双击 Fit 和中键单击临时参考；参考拾取/洋红色覆盖不修改正式 Selection 或 Revision，场景重建与取消会清理。InputManager 在视口捕获时取得焦点，规范化组合按键边沿，Esc/失焦结束手势；文档及验证边界见 [导航说明](../web/apps/cad/src/cad/navigation/README.md)。未以原版软件轨迹标定速度曲线与自动选心，不声明逐像素完全一致。

Three.js 被封装在 Viewport Engine 内，页面层不应直接操作 Scene/Renderer/Controls。统一输入系统处理 Pointer/Keyboard、导航、Tool、Selection、Capture 和 Overlay；全局快捷键由 CommandProvider 统一注册，通过 CommandRegistry 执行并遵循命令可用性；输入框、输入法组合输入、重复按键及打开的对话框不触发全局快捷键。Ctrl/Meta+Z 撤销，Ctrl/Meta+Y 或 Ctrl/Meta+Shift+Z 重做，Ctrl/Meta+K 搜索命令，V 选择、F 适合窗口、1–4 切换标准视图，L/C/R 在草图上下文激活直线/圆/矩形，? 打开快捷键帮助。Enter/Esc 仍保留为多阶段手势的完成/取消输入，工作台底部状态栏显示真实提交状态、当前工具、选择数量、单位与捕获入口，不推断服务端保存成功。Part Design、Sketcher、Assembly、历史与视图命令使用同一套 CAD 语义 SVG 与短名称；Sketcher 将基础几何、几何约束、尺寸约束和常用图形拆为命令分组，常用图形统一为矩形、正六边形和圆。Distance 与 Length 共用一个线性尺寸工具：首次命中直线主体形成 Length，首次命中点后继续选择第二点形成 Distance；Radius、Diameter、Angle 保持独立尺寸类型。约束定义声明按顺序允许的拾取类型/数量、符号、尺寸类型和单位；相切排除 Spline 与 line-line，相等的第二个引用按首个引用限制为 line-line 或 circle/arc pair。Go 命令验证层再次执行相同的类型不变量，UI 过滤不是唯一正确性边界。所有创建/约束按钮统一为单击执行一个逻辑操作后回到选择、双击进入连续模式。Point、Line、Circle、Arc、Polyline、Spline 和 Rectangle 共用完整手势生命周期；Polyline/Spline 以双击或 Enter 结束本次多点采集。创建工具只上报 Point/Line/Circle 三类基础预览几何，由统一策略派生瞬态尺寸、精度和位置；矩形上报两条正交基线、正六边形上报一条代表边，后续复合工具无需自定义尺寸字符串。参考尺寸和持久尺寸 label 都以 CSS pixel 为目标，在每次渲染前根据相机深度/FOV/Viewport 高度反算 world scale，因此缩放长尺寸时保持固定屏幕大小。显示/输入精度统一为长度 2 位、角度 1 位小数；10 mm 网格是吸附步距，`0.01 mm` 是退化几何提交阈值，二者不再混用。尺寸约束按“选择引用 → 从当前已求解几何测量初值 → 移动并点击放置 → 内联编辑 → Enter 提交”的状态机创建；创建后的尺寸可在约束 glyph 或结构树叶节点上双击，并通过同一个编辑器提交 `UPDATE_CONSTRAINT_VALUE`。视口双击由 Pointer 手势状态机基于时间、位移和完整 down/up 序列识别，不依赖浏览器不稳定的 `PointerEvent.detail`。长度、两点距离、半径、直径和夹角均使用实际几何初值，不再要求用户从空值开始输入。

Toolbar 的展示目录由 PostgreSQL `ui_toolbars/ui_toolbar_items` 维护，并通过认证接口 `GET /api/ui/toolbars` 下发工作台归属、锚点、方向、排序、分组、命令稳定 ID、短名称、详细帮助、图标键和连续执行标记。Web 不再以 JSX 决定工具条组成；但服务端目录只是版本化展示配置，命令只有在本地 `CommandRegistry` 注册且当前状态允许时才能执行，未知命令安全地不显示也不可执行。普通 hover 使用统一深色提示显示短名称与已分配的快捷键；上边栏纯图标“这是什么？”进入一次性上下文帮助模式，下一次点击只显示目录中的工具条名称和详细说明，不执行命令。`workbench-command-model` 按工作台过滤并排序目录，再按目录工作台/锚点投影到建模、视图和文档分类；搜索仅包含当前上下文已注册且可见的命令，禁用项保留说明但无法执行。默认命令区固定在视口上方，撤销/重做和视图操作另有常驻入口；旧浮动 Toolbar 布局不再影响默认命令区。界面偏好仍不回写共享目录。

工作台展示层由 `workbench-layout` 管理独立网格列：可调宽/折叠结构树、完整视口和可折叠属性/历史；容器尺寸变化通过 ResizeObserver 更新面板边界，窄窗口属性栏改为覆盖面板。`document-tabs` 常驻展示已打开文档，切换/关闭继续使用原服务端接口。`tree-filter` 保留祖先与 stable key，虚拟树支持方向键、Home/End 和集合选择；筛选不会写入模型或改变稳定身份。面板按钮、树、对话框和输入控件的键盘事件不触发视口工具；原生右键只在 CAD 交互面和树节点上被接管。默认 Inspector 展示文档概览，内部几何 provenance/Worker 信息折叠到技术详情。`workbench-inspector-panel` 独立拥有属性、历史与拓扑查询，关闭时卸载，按页签取数；读取失败明确显示错误，只读状态不提供历史恢复入口。

界面与渲染的语义色源集中在 `design/visual-tokens.ts`，同时生成 CSS 变量、Ant Design 主题与 CAD visual theme。顶栏采用深色、控件采用浅色表面、视口采用蓝灰渐变；选中/悬停/预览分别为琥珀/青/淡紫，求解诊断与坐标轴保持独立语义。实体材质统一低高光及中性照明，点标记与手柄 glyph 使用导数抗锯齿，背景 shader 使用稳定微量抖动降低色带；应用标识与 CAD 命令图标使用原生 SVG。

Insert 使用独立模态资源浏览器，提供全部/最近/共享/文件夹范围、嵌套面包屑、名称与说明检索、类型筛选、排序和每页 12 项分页，选中后展示缩略图、位置与引用方式。数据复用已有带 ACL 的文档检索及缩略图 API；查询状态在客户端，插入边界只提交稳定 DocumentId。当前/根装配禁选，循环引用等由服务端验证；提交防重入，失败保留选择并允许重试。Mock 文件夹面包屑返回完整父链，最近列表基于实际打开时间。
...
命令出现 `sketch solve` 失败时，Web 自动调用 `POST /api/documents/{documentID}/diagnostic-bundles` 并下载 schema `occccad.cad-diagnostic-bundle.v1` JSON；工作台“文档与协作”命令区提供诊断下载，用户可手动下载同一诊断包，即使问题没有触发异常。诊断包包含触发命令及 request ID、当前完整 DocumentView/Sketch、Workspace Head/sequence/policy、历史、最近事务与命令错误、evaluator manifest/digest、客户端环境和日志关联字段，足以在开发环境离线重建候选草图。接口沿用文档 ACL 与 CSRF，响应禁止缓存；Cookie、凭据、其他文档日志和 B-Rep 原始字节不进入导出。
...
- Web 的非权威界面偏好由 schema 5 的 `occccad.ui-preferences.v1` Store 持久化；当前包含 Inspector 开合、命令面板位置、浮动 Toolbar 的位置/方向、结构树宽度、基于稳定结构树 path 的显隐覆盖、鼠标导航模式、CATIA 旋转球显示、捕捉过滤、用户默认显示单位以及按稳定 DocumentId 的文档显示单位覆盖。显示单位只影响 UI 格式和带单位的新输入，Part 求值与几何制品继续使用规范毫米值，已有表达式不被重写；它尚不是团队共享的 Revision 属性。隐藏只控制本地渲染；抑制属于 Revision 中的领域状态。模型、选择和命令状态不得进入客户端偏好契约。结构树位于独立侧栏，使用全高分隔条调宽，支持指针捕获与键盘操作；分支通过折叠/展开箭头呈现，节点与视口仍共用选择身份。文档管理采用全局标题栏中的常驻标签，支持直接切换、关闭和新建。根 Product 不显示无信息的“Product 定义”浮条，只有 occurrence 上下文编辑或独立定义编辑才显示上下文提示。
...
装配移动手柄的视觉尺寸按相机深度保持固定 CSS 像素，当前整体基准尺寸为原始 98 CSS px 的 1.2 倍；显示、轴向拖拽和旋转拖拽共用同一个像素尺度常量，避免第一帧求解回到旧尺寸对应的控制点。轴向和旋转拖拽都以控制图形的目标屏幕位置为约束，通过有限差分 Jacobian 迭代求解当前三维参数。pointermove 只更新候选参数并请求权威预览，不提前修改手柄矩阵；每次成功响应在同一同步更新中应用全部 Instance pose，并以驱动 Instance 相对手势起点的实际 SE(3) 增量更新手柄位置和方向，因此两者始终属于同一确认帧。受限、失败或迟到响应不会推进其中任一方。求解每次按当前相机投影重新计算手柄世界缩放，因此实例沿轴靠近或远离相机时，控制图形仍保持在 pointerdown 点击偏移对应的鼠标射线上；一维自由度无法解释的正交鼠标分量按最小二乘自然忽略。旋转 pointerup 等待最终预览响应后把最后确认的显示方向保存为该 Instance 的新手柄框架，随后权威姿态重置和文档刷新继续使用这一框架；取消手势恢复旋转前框架。移动提交产生的实时刷新在移动工具激活期间保留并重新校验当前 Instance 选择；普通 hover 不再写入待吸附锚点，只有明确点击 Instance 几何或拖动中心吸附时才更新锚点。选择另一个 Instance 会转移手柄，只有明确点击空白或切换工具才会解除。Default 导航在完整模型可见时以可见内容包围盒中心旋转，命中拓扑点时优先使用该点；局部放大时优先使用指针/屏幕中心命中的可见几何，避免围绕远处模型中心甩出当前细节。包含有向角的 C++ 阻尼最小二乘 component 使用确定性回溯线搜索，有向角直接计算连续的 `k·(a×b)`；这使带共线和点重合的真实铰链能跨越 180°收敛到 181°或 270°。
