...
P9A–P9C 已把 Publication 纳入 Part Revision。Datum POINT/AXIS/PLANE/FRAME、PersistentSelection CURVE/SURFACE、Feature output BODY 与 PARAMETER 共用稳定 PublicationId、兼容版本、typed CRUD、ChangeSet、Undo/Redo 和 dependency graph；拓扑 target 随 Part 求值按 naming 重解，失效后保存 `BROKEN_PUBLICATION`。Parameter Publication 固定 value type、dimension、SI unit policy、可选 bounds 与 source ParameterId；消费 Part 的 `ExternalParameterRef` 固定来源 Document、PINNED Revision、PublicationId、期望合同、resolved value 和 digest。普通重算不读取来源 Head；绑定入口验证 Viewer 权限、来源 Revision/Publication、类型/量纲/摘要，并沿实际参数依赖闭包拒绝跨文档循环。显式 Update References 与 `UPDATE_AVAILABLE` 属于 P9D，Product Publication endpoint 属于 P9E。
...
| Part Publication 与跨文档参数 | 已实现 P9A–P9C | Datum/Topology/Feature output/Parameter Publication、稳定合同、解析快照、BROKEN_PUBLICATION、ExternalParameterRef 冻结 Revision/value digest、权限/循环/量纲诊断、结构树/Inspector/管理面板；显式 Update References 与 Product Publication endpoint 尚未实现 |
...
上述装配段落中的“正式 Publication 未实现”现仅指尚未接入 Assembly 的 Product Publication endpoint；Part 内 P9A–P9C Publication 与 ExternalParameterRef 已按本节矩阵实现。
