...
| Angle（角度） | 统一 0–360°有向绕轴角、平行、垂直；显式方向和稳定参考轴 | 当前 ANGLE/DirectedAngle 数学基础可复用；前后端尚需贯通本系统方向/参考轴和几何可用性合同 |
...
- Perpendicular：两方向点积为零，正常构型控制一个转动自由度。与“投影绕轴角等于 90°”明确区分。
- 平面以法向作为方向支持；线面关系必须注明“线与法向”或通过明确的组合表达“线在平面内”，不静默互余转换。无向空间夹角可作为辅助测量显示，但不承载 0–360°方向语义。
