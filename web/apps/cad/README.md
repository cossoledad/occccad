...
- Part Design 的“发布”面板创建和删除 Datum、Face/Edge、Body 与 Parameter Publication；结构树以稳定 PublicationId 显示接口，选中时仍使用实际 target 的视口选择身份定位，并在 Inspector 展示合同、resolved Revision、source digest 与 `BROKEN_PUBLICATION` 诊断。参数面板可把本地参数发布，或用来源 Part 的 Parameter PublicationId 建立冻结 `ExternalParameterRef`；后者显示来源 Revision，普通重算不会隐式跟随来源 Head；
