...
- Product 打开时默认激活根 occurrence；双击树中的 Product/Part/Instance 激活唯一 InstancePath，并在保留根装配和其他部件的场景中就地编辑对应 Reference。Toolbar、属性、历史和命令绑定 Reference Document，草图与预览应用 occurrence 世界 Placement。Insert 写入激活的 Product Reference，InstanceName 由服务端按同级 `ReferenceName.N` 自动分配；`FOLLOW_WORKSPACE_WITH_ACCEPT` 子文档通过递归实时订阅投影 NotUpdated，只有显式“接受并更新引用”才推进 Product 使用的 Revision，PINNED 边截断传播；
