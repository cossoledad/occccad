...
- Product 打开时默认激活根 occurrence；双击树中的 Product/Part/Instance 激活唯一 InstancePath，并在保留根装配和其他部件的场景中就地编辑对应 Reference。Toolbar、属性、历史和命令绑定 Reference Document，草图与预览应用 occurrence 世界 Placement。Insert 写入激活的 Product Reference，InstanceName 由服务端按同级 `ReferenceName.N` 自动分配；默认 `FOLLOW_HEAD` 子文档通过递归实时订阅发现变化，并按叶到根自动提交 `UPDATE_REFERENCES`，重解析装配支持元素并刷新所有 occurrence；当前不显示引用模式与手动接受按钮；
...
- Part Design 的“发布”面板创建和删除 Datum、Face/Edge、Body 与 Parameter Publication；结构树以稳定 PublicationId 显示接口，选中时仍使用实际 target 的视口选择身份定位，并在 Inspector 展示合同、resolved Revision、source digest 与 `BROKEN_PUBLICATION` 诊断。参数面板可把本地参数发布，或用来源 Part 的 Parameter PublicationId 建立冻结 `ExternalParameterRef`；`CURRENT / UPDATE_AVAILABLE / BROKEN` 显示检测状态，接受更新复用显式 `UPDATE_REFERENCES` 命令；
- Assembly Design 可直接选择子 Part Publication 建立约束，并通过“发布”面板转发为 Product Publication；兼容 Replace/Update 重连新 target，不兼容合同显示 Broken。Part 的同一面板可创建 ContextReference：Plane/Axis 物化为 Datum，Parameter 驱动本地参数，Curve 投影到指定草图；Isolate 后成为本地普通值或构造几何；
