...
export function DocumentOrbController() {
  ...
  return <>
    <DocumentOrb documents={(opened.data ?? []).map(({ id, name, type }) => ({ id, name, type }))}
      activeID={activeID} onCreate={() => setCreateOpen(true)}
      onSwitch={(id) => navigate(`/documents/${id}`)} onClose={(id) => void closeDocument(id)} />
    <Modal title="新建文档" open={createOpen} onCancel={() => setCreateOpen(false)} footer={null} destroyOnHidden>
      <Form layout="vertical" initialValues={{ type: "PART" }} onFinish={(values) => void createDocument(values)}>
        <Form.Item name="type" label="文档类型" rules={[{ required: true }]}><Segmented block options={[
