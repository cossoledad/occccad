...
export class ArcSketchTool implements CadTool {
  ...
  pointerMove(event:CadPointerEvent,context:ToolContext):InputResult {
    if(!this.center||event.state.buttons.middle||event.state.buttons.right)return InputResult.Ignored;const value=context.viewport.sketchPoint(event.x,event.y);if(!value)return InputResult.Ignored;
...
abstract class MultiPointSketchTool implements CadTool {
  ...
  pointerMove(event:CadPointerEvent,context:ToolContext):InputResult {if(this.points.length===0||event.state.buttons.middle||event.state.buttons.right)return InputResult.Ignored;const value=context.viewport.sketchPoint(event.x,event.y);if(!value)return InputResult.Ignored;context.viewport.showPolylinePreview([...this.points,value]);return InputResult.Consumed;}
  pointerUp(event:CadPointerEvent):InputResult {if(event.button!==0||event.pointerId!==this.capturedPointerID)return InputResult.Ignored;this.capturedPointerID=undefined;return InputResult.ReleaseCapture;}
